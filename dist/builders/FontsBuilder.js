...
class FontsBuilder {
    ...
    constructor(svgBuilder) {
        ...
        this.fontsPath = (0, path_1.join)(this.svgBuilder.buildOptions.dist, this.svgBuilder.buildOptions.fontName);
        const svgPath = (0, path_1.join)(this.svgBuilder.buildOptions.dist, `${this.svgBuilder.buildOptions.fontName}.svg`);
    ...
    logBatchResults(successful, failed, totalTime) {
        ...
        if (failed.length > 0) {
            ...
            failed.forEach(result => {
                if (result.error) {
                    (0, utils_1.errorLog)(`[FontsBuilder] ${result.format.toUpperCase()} error: ${result.error.message}`);
                }
            });
        }
    }
...
exports.FontsBuilder = FontsBuilder;

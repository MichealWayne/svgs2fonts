 ...
 * @readonly
 */
TemplateRegistry.renderers = new Map([
...
class DemoContentBuilder {
    ...
    addIcon(iconName, unicodeStr) {
        if (!(0, utils_1.isString)(unicodeStr))
            return this;
...
class DemoFileManager {
     ...
     */
    async writeAll() {
        const results = await Promise.all(this.outputTasks.map(async (task) => {
            try {
...
class DemoBuilder {
    ...
    prepareFileOutputs(demoContent) {
        ...
        const unicodeHtml = this.renderTemplate(constant_1.DEMO_HTML, new Map([
            ...templateVars,
        ...
        this.fileManager
            ...
            .addFile((0, path_1.join)(dist, unicodeCssFile), this.renderBaseCSS(fontName), 'Unicode demo CSS');
        // Generate Font class demo files
        const fontClassCssFile = demoFontClassHTML.replace((0, path_1.extname)(demoFontClassHTML), '.css');
        const fontClassCss = this.renderBaseCSS(fontName) + demoContent.classCss;
        const fontClassHtml = this.renderTemplate(constant_1.DEMO_HTML, new Map([
            ...templateVars,
    ...
    handleResults(writeResult, metadata) {
        ...
        if (failed.length === 0) {
            ...
            (0, utils_1.log)(`[success][DemoBuilder] All demo files created successfully! Generated ${metadata.iconCount} icons in ${metadata.generationTime.toFixed(2)}ms`);
            if (this.svgBuilder.buildOptions.debug) {
                (0, utils_1.log)(`[DemoBuilder] Generated files: ${successful
            ...
            (0, utils_1.errorLog)(`[DemoBuilder] Demo generation completed with errors: ${successfulCount} successful, ${failedCount} failed`);
...
exports.DemoBuilder = DemoBuilder;

...
class DefaultBuildObserver {
    ...
    onProgress(current, total, currentFile) {
        if (this.verbose && current % 10 === 0) {
            ...
            (0, utils_1.log)(`[SVGBuilder] Progress: ${progress}% (${current}/${total})${currentFile ? ` - ${currentFile}` : ''}`);
        }
    }
    /**
     * @inheritdoc
    ...
    onComplete(result) {
        if (result.success) {
            (0, utils_1.log)(`[success][SVGBuilder] Font creation completed! Processed ${result.processedCount}/${result.totalCount} files`);
        }
        else {

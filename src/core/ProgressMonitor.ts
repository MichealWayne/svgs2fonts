 ...
 * @LastEditTime 2025-07-27 13:29:11
 */

import { ProgressCallback } from '../types/OptionType';
import { log, warnLog } from '../utils/utils';
...
export interface ProgressMonitor {
   ...
   */
  startTracking(options: { total: number; phase?: string; description?: string } | number): void;

  /**
   * Update progress state
   * @param options Progress update options or number of completed tasks
   */
  updateProgress(options: { completed: number; total?: number; current?: string } | number): void;
 ...
 */
export class DefaultProgressMonitor implements ProgressMonitor {
  private progressState!: ProgressState;
  private phaseTimings: PhaseTimings = {};
  private fileStats!: FileStatistics;
  private memoryStats!: MemoryStats;
  private cacheStats!: CacheStats;
  private progressCallbacks: ProgressCallback[] = [];
  private phaseStartTimes: Map<string, number> = new Map();
  private lastLogTime = 0;
  private logInterval = 1000; // Log progress every 1 second in verbose mode
  ...
  setPhase(phase: string): void {
    ...
    // Log phase change
    log(`[ProgressMonitor] Phase changed to: ${phase}`);
   ...
   * @param verbose Whether verbose mode is enabled
   */
  logProgress(forceLog = false): void {
    const now = Date.now();
  ...
  private notifyCallbacks(): void {
    const progress = {
      ...
      phase: this.progressState.phase as any,
    };
    ...
    this.progressCallbacks.forEach(callback => {
      try {
        ...
        warnLog(`[ProgressMonitor] Progress callback error: ${error}`);
      }
    });
  }
}

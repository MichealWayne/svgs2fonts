...
import { createPerformanceTracker, PerformanceTracker } from './core';
import { BatchModeProcessor, SingleDirectoryProcessor } from './processors';
  ...
  try {
    ...
    const processor =
      validatedOptions.batchMode && validatedOptions.inputDirectories
        ? new BatchModeProcessor(configManager, performanceTracker)
        : new SingleDirectoryProcessor(configManager, performanceTracker);
...
async function setupConfiguration(
  options: Partial<InitOptionsParams> | Partial<EnhancedOptions>
): Promise<{ configManager: ConfigurationManager; performanceTracker?: PerformanceTracker }> {
  // Apply backward compatibility layer to handle legacy options
  const compatLayer = createBackwardCompatibilityLayer();
  ...
  const configManager = createConfiguration(enhancedOptions);
  const validatedOptions = configManager.getOptions();
